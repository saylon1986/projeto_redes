   ...
   if extensao == 'txt':
      ...
      with fitz.open(cmn_arq) as arquivo:
         ...
         for n in range(len(arquivo)):
            texto.append(arquivo[n].get_text())
            if n == 0:
               pg_1.append(arquivo[n].get_text())
...
def Separar_OAB(path):
   ...
   try:
      antigos = pd.read_excel("Pesquisa_redes_procuracao.xlsx", engine ='openpyxl')
      anterior_certos = pd.concat([antigos,df_coletados])
      # print(df_certos)
      anterior_certos.to_excel("Pesquisa_redes_procuracao.xlsx", index = False)
   except:
      df_coletados.to_excel("Pesquisa_redes_procuracao.xlsx", index = False)
   ...
   # if len(df) > 0:
   #    # corta para separar o nome da ação em uma variável própria
   #    cortados = df["Para verificar"].str.split("_", n =1, expand = True)
   #    df["Ação"] = cortados [0]
